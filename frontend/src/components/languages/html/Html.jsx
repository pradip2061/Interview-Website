...
const Html = () => {
  ...
  // ✅ Memoize subitem to prevent unnecessary re-renders
  const memoizedSubitem = useMemo(() => {
    return slideElem[0]?.subItems || [];
  }, [slideElem]);
    ...
    <div className="mt-7 lg:flex">
      ...
      <div className="lg:w-[100%] lg:mr-35">
        {subtopic === 'htmlstructure' ? (
          <HtmlStructure />
        ) : subtopic === 'htmlelements' ? (
          <Elements />
        ) : subtopic === 'htmlattributes' ? (
          <Attributes {...(questions ? { questions } : {})} />
        ) : subtopic === 'htmlformsandinputs' ? (
          <FormInput />
        ) : subtopic === 'htmllinksandnavigation' ? (
          <LinkandNavigation />
        ) : subtopic === 'htmllists' ? (
          <Lists />
        ) : subtopic === 'htmlsemanticelements' ? (
          <SemanticElements />
        ) : subtopic === 'htmltables' ? (
          <Tables {...(questions ? { questions } : {})} />
        ) : subtopic === 'htmlmedia' ? (
          <Media />
        ) : subtopic === 'html5features' ? (
          <Html5 />
        ) : subtopic === 'htmlmetatags' ? (
          <MetaTags />
        ) : (
          <img
            src={img}
            alt="no image found"
            className="w-70 h-70 ml-10 lg:w-100 lg:h-130 lg:ml-80"
          />
        )}
        ...
        <div className="gap-x-1 flex justify-center mt-10">
          <button
            className={`${
              page === 1 ? 'text-white bg-gray-400' : 'text-white bg-blue-500'
            } w-10 h-10`}
            onClick={handlePrev}
            disabled={page === 1}
            aria-label="prev"
          ...
          {Array.from({ length: pages }, (_, index) => (
            <button
              key={index + 1}
              className={`${
                page === index + 1 ? 'bg-blue-900' : 'bg-blue-500'
          ...
          <button
            className={`${
              page === pages ? 'text-white bg-gray-400' : 'text-white bg-blue-500'
            } w-10 h-10`}
            onClick={handleNext}
            aria-label="next"
          >
