...
const Stringandstringmethods = ({ questions }) => {
  ...
  return (
    <div className="h-[40rem] overflow-y-auto w-[100%] scroll-smooth pb-10">
      {questions.map((items, index) => (
        <div key={items?._id}>
          <div className="text-lg font-poppins ml-3 pt-5 text-primary">
            {items?.Topic}
          </div>
          ...
          {items.category === 'output' && (
            <div className="bg-gray-200 w-[90%] p-5 mt-3 ml-4">
              ...
              <button
                ...
                onClick={() =>
                  navigate(`/pythoneditor/${encodeURIComponent(items.outputQuestion)}`)
                }
          ...
          <div
            className="flex gap-2 items-center w-[90%] h-10 bg-gray-200 pl-3 mt-3 ml-4"
...
export default React.memo(Stringandstringmethods);
