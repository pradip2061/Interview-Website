...
const createPythonQuestionOutput = async (req, res) => {
  ...
  const data = await PythonTheory.findOne({Topic:Topic})
  console.log(data)
  if(!data){
    const queries = new PythonTheory({
        Topic:Topic,
        data:[
            {
                question:question,
...
const createPythonQuestionTheory = async (req, res) => {
    const{Topic,question,theory,category,code}=req.body
    if(!Topic|| !question|| !category){
      return res.status(400).json({
          message:'All field are required!'
      })
    }
  ...
  const getpythonquestion = async (req, res) => {
    try {
        ...
        const topicDoc = await PythonTheory.findOne({ Topic: topics }); // Make sure key is `Topic`, not `topics`
...
module.exports = {createPythonQuestionOutput,getpythonquestion,createPythonQuestionTheory}